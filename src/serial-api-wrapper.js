...
class SerialApiWrapper {
  ...
  connect (bitrate) {
    var serialApiWrapper = this;
    var options = this.connectionOptions;
    var serialApi = this.serialApi;
    var devicePath = this.devicePath;
  ...
  // data must be in the form of an ArrayBuffer
  send (data) {
    // validate data is an array buffer
    var id = this.connection.id;
    var serialApi = this.serialApi;

    return new Promise( function (resolve) {
      serialApi.send(id, data, function (res) {
        console.log(res);
        resolve(res);
      });
    });
